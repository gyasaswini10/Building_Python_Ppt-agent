    ...
    try:
        ...
        async with httpx.AsyncClient(headers=_HTTP_HEADERS, timeout=10.0, follow_redirects=True) as client:
            ...
            if resp.status_code == 200:
                ...
                scored = sorted([(self._score_sentence_for_slide(p, topic_display, slide_title), p) for p in points], key=lambda x: -x[0])
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if r.status_code == 200:
                data = r.json()
                pts = [d.get("definition") for m in data[0].get("meanings", []) for d in m.get("definitions", [])]
